...
快速排序的最优情况是每一次取到的元素都刚好平分整个数组。此时时间复杂度的公式则为：$T[n]=2T[n/2]+f(n)$，$T[n/2]$为平分后的子数组的时间复杂度，$f(n)$为评分这个数组所花的时间。
...
于是得到：$T[n/(2^m)]=T[1]$，则有：$n=2^m$，即：$m=\log(n)$。

故有：$T[n] = 2^{\log(n)} T[1] + n \log(n)$，其中$n$为元素个数，当$n \geq 2$时，$n \log(n) \geq n$，所以取后面的$n\log(n)$；

综上所述：快速排序最优情况下时间复杂度为：$\Omega(n \log(n))$。
